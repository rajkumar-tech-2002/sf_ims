...
const InvoicePrint = ({ invoiceDate, invoiceNo, customer, items, totals, gstMode }) => {
    return (
        <div className="print-only hidden print:block bg-white text-slate-900 p-8 w-full max-w-[210mm] mx-auto min-h-[297mm]">
            ...
            <div className="flex justify-between items-start border-b-2 border-slate-900 pb-6 mb-8">
                ...
                <div className="text-right">
                    <h2 className="text-xl font-bold uppercase tracking-widest text-slate-900 mb-2">TAX INVOICE</h2>
                    <div className="space-y-1">
                        <p className="text-xs text-slate-500 font-bold uppercase">Invoice No: <span className="text-slate-900">{invoiceNo}</span></p>
                        <p className="text-xs text-slate-500 font-bold uppercase">Date: <span className="text-slate-900">{new Date(invoiceDate).toLocaleDateString()}</span></p>
                    </div>
                </div>
            </div>
