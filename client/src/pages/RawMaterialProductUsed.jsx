...
const RawMaterialProductUsed = () => {
    ...
    const fetchInitialData = async () => {
        try {
            ...
            const [usageRes, stockRes] = await Promise.all([
                api.get('/raw-material-product-used'),
                api.get('/raw-material-stocks')
            ]);
            setUsages(usageRes.data);
            setStocks(stockRes.data);
        } catch (error) {
            showToast('error', 'Failed to fetch raw material data');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await api.post('/raw-material-product-used', formData);
            showToast('success', 'Raw material usage recorded successfully');
            resetForm();
            fetchInitialData();
        } catch (error) {
    ...
    const handleDelete = (id) => {
        confirmToast('Delete this usage record and revert stock?', async () => {
            try {
                await api.delete(`/raw-material-product-used/${id}`);
        ...
        <div className="p-6 lg:p-10 bg-slate-50 min-h-screen">
            <div className="max-w-7xl mx-auto space-y-8">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 className="section-title text-2xl font-bold text-slate-800">Raw Material Consumption</h1>
                        <p className="text-slate-500 text-base mt-2 font-medium">Record and subtract raw materials used for production.</p>
                    </div>
                    <button
                    ...
                    <form onSubmit={handleSubmit} className="p-8 space-y-8">
                        ...
                        <div className="flex flex-wrap items-center gap-10 bg-rose-50/50 p-6 rounded-2xl border border-rose-100">
                            <div className="flex items-center gap-4 pr-10 border-r border-rose-100">
                                ...
                                <div className="flex flex-col">
                                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Material Code</span>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            ...
                            <div className="space-y-2 lg:col-span-3">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 flex items-center gap-2">
                                    <FileText size={12} /> Usage Description / Note
                                </label>
                                <textarea
                                    name="description"
                                    className="input-field min-h-[100px] py-4 font-medium"
                                    value={formData.description}
                                    onChange={handleInputChange}
                                    placeholder="e.g. Used for batch #102 production..."
                                />
                            </div>
                        </div>
...
export default RawMaterialProductUsed;
